import React, {
  ...
  function Picker({
    ...
    useEffect(() => {
        ...
        if (valueNotNull) {
            if (multiple) {
              if (value.every(obj => typeof obj === 'object' && obj !== null)) {
                  initialSelectedItems = items.filter(item1 => value.some(item2 => deepEqual(item1.value, item2)));
              } else {
                  initialSelectedItems = items.filter(item => item[_schema.value] === value);
              }
            } else {
    ...
    useEffect(() => {
        if (multiple) {
            setNecessaryItems(state => {
                ...
                let _state = []
                if (value.every(obj => typeof obj === 'object' && obj !== null)) {
                  _state = [...state].filter(item1 => value.some(item2 => deepEqual(item1.value, item2)));
                } else {
                   _state = [...state].filter(item => item[_schema.value] === value);
                }
  
                const newItems = value.reduce((accumulator, currentValue) => {
                  let index = -1;
                  if (typeof currentValue === 'object' && currentValue !== null) {
                      index = _state.findIndex(item => deepEqual(currentValue, item[_schema.value]));
                    } else {
                      index = _state.findIndex(item => item[_schema.value] === currentValue);
                    }
  ...
  // Filter Chips
    ...
    const selectedItems = useMemo(() => {
        if (! multiple)
          ...
          return necessaryItems.filter(item => {
              ...
              if (Array.isArray(_value) && _value.every(obj => typeof obj === 'object' && obj !== null)) {
                  return _value.some(obj => deepEqual(obj, item[_schema.value]));
              } else {
    ...
    const onPressBadge = useCallback((value) => {
        setValue(state => {
            let _state = [...state];
            let _unique = [
              ...new Map(_state.map((item) => [item?.key, item])).values(),
            ];
  
            const index = _unique.findIndex(item => deepEqual(item, value));
            _unique.splice(index, 1);
    ...
    function deepEqual(obj1, obj2) {
      ...
      // Check if the values of each property are deeply equal
    ...
    const __renderListItem = useCallback(({item}) => {
        ...
        if (multiple) {
          if (Array.isArray(_value) && _value.every(obj => typeof obj === 'object' && obj !== null)) {
              isSelected = _value.some(obj => deepEqual(obj, item[_schema.value]));
          } else {
              isSelected = _value.includes(item[_schema.value]);
          }
        } else {
          if(typeof(_value) === "object") {
              isSelected = deepEqual(_value, item[_schema.value]);
          } else {
